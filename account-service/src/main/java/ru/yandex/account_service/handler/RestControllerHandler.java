package ru.yandex.account_service.handler;
...
public class RestControllerHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> handleError4xxException(ApiServiceException e) {
        log.debug("ApiServiceException handler: {}", e.getMessage());
        return ResponseEntity
            .of(buildProblemDetail(e.getMessage(), e.getHttpStatus()))
            .build();
    }
